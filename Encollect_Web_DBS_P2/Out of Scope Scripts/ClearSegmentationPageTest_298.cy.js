...
describe('Clear Segmentation - Clear Segment', () => {
    let loginPage;
    ...
    it('Clear Segmentation - TC_ID_298 - Clear the segmentation by selecting the segmentation name in Auto predictive text', () => {
        getTestData('loginData', 'login').then(user => {
            loginPage.login(user.Companyname, user.email, user.password);
                cy.wait(10000);
                clearsegmentationPage.segmentationModule();
                clearsegmentationPage.ClearSegmentation298();
                //loginPage.logout();     
            });
        });
    });